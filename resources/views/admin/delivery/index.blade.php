                        ...
                        @forelse ($deliveries as $index => $delivery)
                            <tr>
                                ...
                                <td class="text-center">
                                    <span
                                        class="badge bg-{{ $delivery->delivery_status == 'Delivered' ? 'success' : ($delivery->delivery_status == 'In Transit' ? 'info' : ($delivery->delivery_status == 'Cancelled' ? 'danger' : 'secondary')) }}">
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="#" class="btn btn-sm btn-outline-secondary me-1" data-bs-toggle="modal"
                                        data-bs-target="#deliveryModal{{ $delivery->delivery_id }}">
                                    ...
                                    {{-- Delete --}}
                                    <form action="{{ route('delivery.delete', $delivery->delivery_id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirm('Are you sure you want to delete this delivery?')"
                ...
                <div class="modal-content shadow border-0 rounded-4">
                    ...
                    <div class="modal-header bg-primary text-white py-2 px-3 rounded-top-4">
                        <h5 class="modal-title">
                            <i class="fa-solid fa-truck me-2"></i> Delivery Details
                        </h5>
                    ...
                    <div class="modal-body p-3 text-center">
                        ...
                        <ul class="list-unstyled text-start mb-0">
                            <li class="mb-2"><strong>Customer:</strong> {{ $delivery->customer->name ?? '-' }}</li>
                            <li class="mb-2"><strong>Order:</strong> #{{ $delivery->order->order_number ?? '-' }}</li>
                            <li class="mb-2"><strong>Address:</strong> {{ $delivery->delivery_address }}</li>
                            <li class="mb-2"><strong>Status:</strong>
                                <span
                                    class="badge bg-{{ $delivery->delivery_status == 'Delivered'
                                        ? 'success'
                            ...
                            <li class="mb-0"><strong>Time:</strong> {{ $delivery->delivery_time ?? '-' }}</li>
                        </ul>
                    </div>
